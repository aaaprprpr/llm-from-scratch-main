...
import math
from torchtune.modules import FeedForward,TransformerSelfAttentionLayer,MultiHeadAttention,RMSNorm,RotaryPositionalEmbeddings
...
class Block(nn.Module):
    def __init__(self, d_model:int,n_head:int, d_ff:int,theta:float=10000.0):
        ...
        rope=RotaryPositionalEmbeddings(dim=head_dim, max_seq_len=None, base=theta)
        attn=MultiHeadAttention(
            embed_dim=d_model,
            num_heads=n_head, 
            num_kv_heads=n_head//4, 
            pos_embeddings=rope,
            bias=False, 
            is_causal=True,
        )
        ...
        sa_norm=RMSNorm(d_model)
        mlp_norm=RMSNorm(d_model)
        self.block=TransformerSelfAttentionLayer(
            attn=attn,
            mlp=ffn,
            sa_norm=sa_norm,
            mlp_norm=mlp_norm
    ...
    def forward(self, x: torch.Tensor, token_positions: torch.Tensor | None = None) -> torch.Tensor:
        return self.block(x,mask=None, input_pos=token_positions)
...
class Transformer(nn.Module):
    ...
    def forward(self, 
        ...
        for layer in self.layers:
            x = layer(x, input_pos=token_positions) # 注意力
        x = self.norm(x)
